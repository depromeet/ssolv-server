...
/**
 * meeting_calculation_stream 의 idle 메시지를 XCLAIM 으로 소유권 이전 후 직접 재처리하는 복구 스케줄러.
 *
 * 흐름:
...
@Component
@Profile("!test")
class PlaceStreamRecoveryScheduler(
    private val stringRedisTemplate: StringRedisTemplate,
    private val executePlaceSearchService: ExecutePlaceSearchService,
) {
    private val logger = LoggerFactory.getLogger(PlaceStreamRecoveryScheduler::class.java)

    // 인스턴스별 고정 consumer 이름
    ...
    fun recoverPendingMessages() {
        val requestId = "place-recovery-" + UUID.randomUUID().toString().substring(0, 8)
        ...
        try {
            ...
            logger.debug("[place-recovery] idle 메시지 {} 건 발견 → XCLAIM 시작", idleMessages.size)
            ...
            claimed.forEach { message ->
                ...
                if (meetingId == null) {
                    ...
                    logger.warn("[place-recovery] 파싱 불가 메시지 폐기 (id: {})", message.id)
                    return@forEach
                }
                ...
                runBlocking {
                    try {
                        ...
                        stringRedisTemplate.opsForStream<String, String>()
                            .acknowledge(streamKey, groupName, message.id)
                        logger.debug("[place-recovery] 재처리 성공 (meetingId: {})", meetingId)
                    } catch (e: Exception) {
                        logger.error("[place-recovery] 재처리 실패 (meetingId: $meetingId)", e)
            ...
            logger.error("[place-recovery] 스케줄러 실행 중 예외 발생", e)
            Sentry.captureException(e)
        } finally {
            MDC.clear()
        }
