#!/bin/bash
# Claude Code 텔레메트리 이벤트 → Grafana Cloud Loki 직접 푸시
#
# 사용법: telemetry-emit.sh <event_name> [key=value ...]
# 예시:   telemetry-emit.sh skill.invoked skill_name=git-conventions
#         telemetry-emit.sh hook.blocked hook_name=commit-msg-check reason=non-ascii
#
# 필요 환경변수 (~/.claude/settings.json env 블록):
...
# Loki 직접 푸시 (동기, timeout=3s — 훅 지연 최소화)
...
payload = json.dumps({
    "streams": [{
        "stream": {"service_name": "claude-code", "developer": developer, "event": event_name},
