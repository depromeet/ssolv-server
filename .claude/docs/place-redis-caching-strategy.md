...
## 6. 중복 요청 방지 전략 (Cache Stampede 방어)

동일한 모임ID(`meetingId`)에 대해 여러 명의 사용자가 동시에 검색 결과를 요청할 경우, 고비용의 Google Places API가 중복으로 호출되는 현상(Thundering Herd)을 방지하기 위해 **분산 락을 이용한 Double-Check** 전략을 사용합니다.

### 6.1 처리 프로세스
1.  **1차 확인**: Redis 캐시(`meeting:places:{mId}`)가 존재하는지 확인합니다.
2.  **락 획득 시도**: 캐시가 없을 경우, `place_search_lock:{mId}` 키로 분산 락(10초 TTL)을 획득하려 시도합니다.
3.  **2차 확인 (Double-Check)**: 락을 획득한 직후, 다른 요청이 이미 데이터를 생성했는지 다시 한번 캐시를 확인합니다.
4.  **Google API 호출**: 여전히 데이터가 없는 경우에만 실제 Google API 호출 및 데이터 정규화 저장을 수행합니다.
5.  **락 획득 실패 시 처리**: 이미 다른 요청이 작업 중이므로, 지수적 대기(Exponential Backoff)를 수행하며 캐시가 채워지기를 기다린 후 결과를 반환합니다.

### 6.2 왜 Watchdog을 쓰지 않나요? (API vs Batch)
*   **Batch**: 수십 분 이상 걸리는 예측 불가능한 배치 작업은 락 만료를 방지하기 위해 **Watchdog(실시간 TTL 연장)**이 필수적입니다.
*   **API**: 구글 API 타임아웃이 5초 내외로 명확하므로, **10초 정도의 고정 TTL 락**만으로도 충분히 안전하며 리소스를 절약할 수 있습니다.