...
interface MeetingAttendeeJpaRepository : JpaRepository<MeetingAttendeeEntity, Long> {
    ...
    @Query(
        "SELECT ma FROM MeetingAttendeeEntity ma JOIN FETCH ma.user JOIN ma.meeting m JOIN ma.user u WHERE m.id = :meetingId AND u.id = :userId",
    )
    fun findByMeetingIdAndUserId(@Param("meetingId") meetingId: Long, @Param("userId") userId: Long): MeetingAttendeeEntity?
    ...
    @Query(
        "SELECT CASE WHEN COUNT(ma) > 0 THEN true ELSE false END FROM MeetingAttendeeEntity ma JOIN ma.meeting m JOIN ma.user u WHERE m.id = :meetingId AND u.id = :userId",
    )
    fun existsByMeetingIdAndUserId(@Param("meetingId") meetingId: Long, @Param("userId") userId: Long): Boolean

    @Query(
        "SELECT CASE WHEN COUNT(ma) > 0 THEN true ELSE false END FROM MeetingAttendeeEntity ma JOIN ma.meeting m JOIN ma.user u WHERE m.id = :meetingId AND ma.attendeeNickname IS NOT NULL AND ma.attendeeNickname = :nickname AND u.id != :excludeUserId",
    )
