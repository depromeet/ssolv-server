...
class DeviceTokenQueryRepositoryImpl(
    ...
    override fun findByUserId(userId: Long): List<DeviceToken> = deviceTokenJpaRepository.findByUserId(userId)
        .map { deviceTokenMapper.toDomain(it) }

    override fun findByFcmToken(fcmToken: String): DeviceToken? = deviceTokenJpaRepository.findByFcmToken(fcmToken)
    ...
    override fun findByUserIdAndPlatform(userId: Long, platform: org.depromeet.team3.notification.domain.DevicePlatform): DeviceToken? =
        deviceTokenJpaRepository.findByUserIdAndPlatform(userId, platform)
            ?.let { deviceTokenMapper.toDomain(it) }
